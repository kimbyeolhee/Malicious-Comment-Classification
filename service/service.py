...
#학습 모델 로드
path = "../checkpoint/saved.pt"

device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
model = MCCClassifier(num_classes=5, dr_rate = 0.45).to(device)
...
#초기 사용자 데이터
userdata = pd.read_excel('../data/userdata.xlsx')
ID = ''
...
@app.route('/')
def start_screen():
	return render_template('./templates/start_screen.html')
...
def predict_toxin():
    ...
    if pred == []:
        ...
        userdata.to_excel('./userdata.xlsx', index=False)
        return render_template('./templates/alert2.html', max_troll = max_troll, max_columns = max_columns, ID = ID, new = new, comment = test_comment, dom = dom)

    else:
        userdata['악성댓글'][id_index] = userdata['악성댓글'][id_index] + 1
        userdata.to_excel('./userdata.xlsx', index=False)
        return render_template('./templates/alert1.html', category = category, max_troll = max_troll, max_columns = max_columns, ID = ID, new = new, comment = test_comment, dom = dom)
